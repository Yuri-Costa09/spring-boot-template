...
1. **Create app**:
   ...
   ```bash
   heroku create template-spring-boot
   ```
...
3. **Set environment variables**:
   ...
   ```bash
   heroku config:set APP_NAME=template-spring-boot
   heroku config:set JWT_PRIVATE_KEY=file:/app/secrets/app.key
   ```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: app
        image: template-spring-boot:latest
        ports:
        - containerPort: 8080
        env:
        - name: DATABASE_URL
...
```bash
# Backup
docker exec template-spring-boot-db-prod pg_dump -U postgres template_db > backup.sql

# Restore
docker exec -i template-spring-boot-db-prod psql -U postgres template_db < backup.sql
```
...
For more information, see the [Spring Boot Documentation](https://docs.spring.io/spring-boot/docs/current/reference/html/).
